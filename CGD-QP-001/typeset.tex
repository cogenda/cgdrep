...
一般的表格、图片和代码都当有一对\filename{Caption} 和\filename{Label}。\filename{Caption} 是这个表格（或图片、代码）的名字，其字号比正文小半号。\filename{Label} 是引用它时用的代号，\filename{Label} 的内容并不出现在排版后的\filename{pdf}页面上。以下分别讲如何实现。

\section{表格}
一般，我们要求表格采用三线表，首行为其\filename{header}，用黑体。字号比正文小半号，位置居中。如表~\ref{triline}：
...
实现这样的表及其浮动位置，可以用 Cogenda 自己制作的宏 \filename{ctable}，其语法为：
...
如果你不想局限于 \filename{ctable} 的功能，也可以用一般的 \LaTeX 表格语法（用了 \filename{tabu} 宏包），表~\ref{triline} 的一般\LaTeX 语法为：
...
\subsection{参考文献}
参考文献的内容写入文件 \filename{main.bib} 中，格式示例：
